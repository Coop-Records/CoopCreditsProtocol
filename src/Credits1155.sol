    ...
    ERC1155BurnableUpgradeable
{
     ...
     */
    error Credits1155_MultiTokenDropMarket_Invalid_Sales_Term_Id(
        uint256 saleTermsId
    );
     ...
     */
    error Credits1155_Insufficient_Credits_Balance(
        uint256 required,
        uint256 available
    );
     ...
     */
    error Credits1155_Insufficient_ETH_In_Contract(
        uint256 required,
        uint256 available
    );
    ...
    function mintWithCredits(
        ...
        if (fixedPriceSale.multiTokenContract == address(0)) {
            revert Credits1155_MultiTokenDropMarket_Invalid_Sales_Term_Id(
                saleTermsId
            );
        }

        // Burn balance and mint with unlocked ETH
        uint256 creditsCost = getCreditsCostForMint(
        ...
        if (creditsCost > userCreditsBalance) {
            revert Credits1155_Insufficient_Credits_Balance(
                creditsCost,
                userCreditsBalance
            );
        }
    ...
    function adminWithdraw(
        ...
        if (amount > contractBalance) {
            revert Credits1155_Insufficient_ETH_In_Contract(
                amount,
                contractBalance
            );
        }
     ...
     */
    function getCreditsCostForMint(
        uint256 pricePerQuantity,
        uint256 tokenQuantity
    ) public pure returns (uint256 creditsCost) {
